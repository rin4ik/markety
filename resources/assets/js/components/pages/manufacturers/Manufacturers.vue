        ...
        <div class="page-body">
            <table id="myTable" class="table table-striped table-hover table-sm">
                <thead>
                 ...
                 <Manufacturer @render="render" :deletedItem="deleted" @selected="markSelected" @unselected="unselect" :checked="allItems" v-for="(manufacturer,index) in all" :key="index" :manufacturer="manufacturer" /> 
...
export default {
    ...
    methods: {
        ...
        change() {
            if(this.allItems){
                if(this.value == 2){
                    axios.delete('/admin/remove/manufacturers')
                    .then((response)=>{
                        this.all=''
                    ...
                    this.selectedItems.map((item, key) => {  
                        this.all.map((man, index) =>{
                            if(man.id == item.id){
                                ...
                                axios.delete('/admin/manufacturers/' + item.id).then(()=>
                                {
